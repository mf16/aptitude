...
(function(a) {
    a.fn.editorAptitude = function(t) {
                        ...
                        if ("m" != f || "," != q)
                            if ("=" != f || "m" != q) {
                                ...
                                if ("mlinebreak" == c.classname) n++, x++;
                                    ...
                                    "left" == m ? a(G).text(f.substring(0, f.length - (n - x))) : a(G).text(f.substring(0, f.length - (n - x) + 1));
                                    /* WTF Bug?
                                    "transparent" != e.activebg && ("mo" != c.classname && "mtext" != c.classname) && a("#" + c.id).css("background-color", e.activebg);
                                    "transparent" != e.activebg && ("mo" != c.classname && "mtext" != c.classname) && a("#" + c.id).css("background-color", e.activebg + " !important");
									FIX RIGHT BELOW
									*/
                                    $('#'+c.id).css('background-color', e.activebg);
                                    ...
                                    return !1
                                    $('#aptitudeCursor').css('display', 'none');
                                }
                            }
                    q = c.text
                })
        ...
        e.emptybg = "#D8D8D8";
        e.iconpath = "<?php echo $_SERVER['DOCUMENT_ROOT']; ?>img/editor";
        e.onfocus = null;
        e.onblur = null;
...
//Animation for the operator toolbar
$(document).click(function(event) {
	//get the id of the clicked element, that way if they click on a button it still maintains focus on the bar
    var clickedElement = $(event.target).attr('class');
    
});
$( "#editor" ).focusin(function() {
	$(".operatorToolbar").animate({top:"0px"});
});
//DOESNT WORK, FIGURE OUT HOW TO GET TOOLBAR TO STAY
$( "#editor" ).focusout(function() {
	if (clickedElement !== 'operatorToolbar'){	
		$(".operatorToolbar").animate({top:"-100px"});
	}
});
