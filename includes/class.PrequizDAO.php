...
class PrequizDAO {
	...
	function getNextQuestion($subjectName,$chapterid,$sectionid,$type){
		...
		$sql="SELECT problem_id,concept_id,problem,concept_hack,answer,problem_type FROM math.problems WHERE chapter_id=? AND section_id=? ".$usedProblemidString.$typeString." ORDER BY RAND() LIMIT 1";
	...
	function getProblemByid($problemid){
		global $db;
		$sql="SELECT problem_id,chapter_id,section_id,concept_id,problem,concept_hack,answer,domain,problem_type FROM math.problems WHERE problem_id=?";
